...
describe(`\x1b[46m bee-alias \x1b[0m`, () => {
  describe(`the prop of "placeholder"`, () => {
    ...
    it(`the placeholder is invisible when the value is not empty.`, async () => {
      wrapper.setProps({
        value: 'value'
      })
  ...
  describe(`the prop of "maxWidth"`, () => {
    it(`should be equal seted`, async () => {
      ...
      const optionWrapper = createWrapper(wrapper.vm._optionsInstance)
      expect(optionWrapper.find('.bee-popper--body').find(':first-child')
        .attributes('style')).to.include(`max-width: ${_maxWidth}`)
    })
  })
