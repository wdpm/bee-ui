export default {
  ...
  /**
   * 这个函数定义很混乱。
   * 1. 如果提供了 type，那么是判断data的类型和type字符串是否相等。
   * 2. 如果不提供 type，就是返回data的类型
   *
  ...
  /**
   * Remove the empty property of an object.
   * @param {Object} object
   * @returns {Object}
   */
  clearEmpty (object) {
    // 内部cache机制，每一个创建都是独立的cache，保存到闭包中
    let caches = []
